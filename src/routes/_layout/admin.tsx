...
const INITIAL_APPOINTMENTS: Appointment[] = [
  ...
  },
  {
    ...
    service: "State Inspection & Emissions",
  ...
  },
  {
    ...
    service: "Oil & Preventative Maintenance",
...
const SERVICE_PIE_DATA = [
  { name: "Collision & Auto Body", value: 45, color: "var(--primary)" },
...
function AdminPage() {
  ...
  const [appointments, setAppointments] = useState<Appointment[]>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("canton_admin_appointments");
      if (saved) {
        try {
  ...
  const [repairOrders, setRepairOrders] = useState<RepairOrder[]>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("canton_admin_repair_orders");
      if (saved) {
        try {
          return JSON.parse(saved);
        } catch (e) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem("canton_admin_appointments", JSON.stringify(appointments));
    }
  }, [appointments]);

  useEffect(() => {
    if (typeof window !== "undefined") {
      localStorage.setItem("canton_admin_repair_orders", JSON.stringify(repairOrders));
    }
  }, [repairOrders]);
          ...
          <TabsContent value="loaners" className="space-y-6">
            <Card className="border-border shadow-sm">
              <CardHeader>
                ...
                <CardDescription className="text-xs">
                  Eli's shop provides free courtesy loaner vehicles during major repairs. Track availability and returns below.
                </CardDescription>
              </CardHeader>
              <CardContent>
