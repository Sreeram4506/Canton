        ...
        <div className="mt-10 divide-y divide-border border-t border-border">
          {NAV.map((item, i) => (
            <Reveal key={item.href} delay={i * 0.06}>
              <Link
                to={item.href}
                className="group flex items-center justify-between gap-6 py-6 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
                ...
                <ArrowUpRight className="h-6 w-6 shrink-0 text-muted-foreground transition-all duration-200 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-primary" />
              </Link>
            </Reveal>
          ))}
        </div>
